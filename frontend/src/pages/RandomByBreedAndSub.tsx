...
const RandomByBreedAndSub = () => {
  ...
  const searchDogs = async () => {
    ...
    if (listSub.length === 0) {
      ...
      const url = `http://localhost:3000/breeds/subBreed/random`;
      await axios
        .post(
          url,
          {
            breed: input.toLowerCase(),
            subBreed: inputSub.toLowerCase(),
          },
          { headers: authHeader() }
        )
        .then((response) => {
  ...
  return (
    <div className="bg-white w-full h-full flex flex-col justify-center items-center">
      ...
      <div className=" top-10 w-11/12 h-2/4 flex justify-center items-start">
        <div
          className={`w-72 h-80 bg-black overflow-hidden rounded-lg shadow-lg ${displayDiv}`}
        >
          <img className="w-full h-full object-cover" src={randomDog} alt="" />
        </div>
      </div>
    </div>
  );
...
export default RandomByBreedAndSub;
